...
import {
  ...
  searchCanonicalLocations,
} from "../service/locations.js";
...
app.post("/findflat", cors(corsOption), async (req, res) => {
  ...
  try {
    const locationId =
      req.body?.locationId == null ? null : Number(req.body.locationId);
    if (locationId !== null && !Number.isInteger(locationId)) {
      return res.status(400).json({ error: "locationId must be an integer" });
    }

    const selectedLocation =
      locationId === null ? null : await getCanonicalLocation(locationId);
    ...
    // Send message first
    res.write(
      `data: ${JSON.stringify({ type: "message", content: result.message })}\n\n`,
    );
    ...
    if (result.listings && result.listings.length > 0) {
      for (const listing of result.listings) {
        res.write(
          `data: ${JSON.stringify({ type: "listing", data: listing })}\n\n`,
        );
