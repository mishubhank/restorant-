...
export async function resolveLocationAliases(
  ...
  for (const row of data ?? []) {
    ...
    if (row.alias_key && area?.name) aliases.set(row.alias_key, area.name);
  }
...
export async function searchCanonicalLocations(
  query: string,
  ...
  return (data ?? []) as CanonicalLocation[];
}
...
export async function getCanonicalLocation(
  ...
  if (error) throw error;
  return data as CanonicalLocation | null;
}
...
export async function createCanonicalLocation(
  ...
  const { data, error } = await supabase
    .from("locations")
    .upsert(
      { name: cleanName, city: cleanCity, latitude: null, longitude: null },
