...
export function formatListings(rawListings: any[]): FormattedResponse {
  if (!rawListings || rawListings.length === 0) {
    return {
      type: "results",
      message:
        "No available listings match that search yet. Try a nearby area, a higher budget, or create an alert. Posts from people looking for a home are kept separate from available listings.",
      count: 0,
      listings: [],
    };
  }
...
function formatListing(listing: any): FormattedListing {
  const priceRange =
  ...
  if (listing.food_preference) {
    ...
    preferences.push(
      foodMap[listing.food_preference] || listing.food_preference,
    );
  }
  ...
  const imageUrl =
    normalizeUrl(listing.image_url) ?? normalizeUrl(listing.thumbnail_url);
  const thumbnailUrl =
    normalizeUrl(listing.thumbnail_url) ?? normalizeUrl(listing.image_url);
