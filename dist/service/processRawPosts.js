...
async function processRawPosts() {
    ...
    for (const post of rawPosts) {
        ...
        const listing = await answerMyQuestion(post.raw_text, normalizedImageUrl ?? undefined);
        ...
        try {
            await listingsRepository.saveListing({
                ...
                source_post_url: normalizedSourceUrl && normalizedSourceUrl.startsWith("http")
                    ? normalizedSourceUrl
                    : null,
            ...
            console.log("✓ Listing saved and marked as processed for post:", post.post_id);
        }
        catch (error) {
            if (error?.code === "23505") {
                // Duplicate key error - mark as processed anyway
                console.warn(`⚠️ Duplicate listing for post ${post.post_id}, marking as processed`);
                await rawPostsRepository.markProcessed(post.post_id);
            }
            else {
                console.error(`❌ Error saving listing for post ${post.post_id}:`, error?.message);
...
//# sourceMappingURL=processRawPosts.js.map